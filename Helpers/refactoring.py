...
os.environ["LANGCHAIN_PROJECT"] = "YOUR-LANGCHAIN-PROJECT"
...
task_verifier_model = ChatOpenAI(model="gpt-5.1", temperature = 0.5)
refactor_model = ChatOpenAI(model="gpt-5.1", temperature = 0.2)
...
def get_response_gpt_postprocessing(user_content):
    import os
    ...
    try:
        ...
        return f"Error: Failed to get response from GPT: {str(e)}"
    finally:
    ...
    if issues:
        ...
        question = f"""
            ...
            *** ONLY Output the final fixed design(new declaration + attachments) ***
            """
        response = get_response_gpt_postprocessing(question)
        print(response)
